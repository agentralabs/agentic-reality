...
pub struct ResponseMetrics { pub layer: Layer, pub tokens_used: u64, pub tokens_saved: u64, pub cache_hit: bool }
...
pub struct TokenMetrics { pub total: AtomicU64, pub cache_savings: AtomicU64, pub scope_savings: AtomicU64, pub delta_savings: AtomicU64 }
impl TokenMetrics {
    ...
    pub fn record(&self, layer: Layer, tokens: u64, potential: u64) {
        ...
        match layer { Layer::Cache => { self.cache_savings.fetch_add(saved, Ordering::Relaxed); } Layer::Scoped => { self.scope_savings.fetch_add(saved, Ordering::Relaxed); } Layer::Delta => { self.delta_savings.fetch_add(saved, Ordering::Relaxed); } _ => {} }
    }
    ...
    pub fn conservation_score(&self) -> f64 { let t = self.total_tokens(); let s = self.total_savings(); let p = t + s; if p == 0 { 1.0 } else { s as f64 / p as f64 } }
}
