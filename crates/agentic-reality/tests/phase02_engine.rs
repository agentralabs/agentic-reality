...
fn test_set_role() {
    ...
    let role = DeploymentRole {
        name: "primary".into(),
        responsibilities: vec!["serve".into()],
...
fn test_update_cardinality() {
    ...
    engine
        .writer()
        .update_cardinality(Cardinality::ReplicaOf { total: 3, index: 0 })
        .unwrap();
    let reader = engine.reader();
    let nature = reader.get_nature().unwrap();
    assert!(matches!(
        nature.cardinality,
        Cardinality::ReplicaOf { total: 3, index: 0 }
    ));
}
...
fn test_record_wisdom() {
    let mut engine = RealityEngine::new();
    engine.writer().initialize_soul(make_soul()).unwrap();
    let wisdom = IncarnationWisdom {
        lesson: "always check health".into(),
        learned_from: IncarnationId::new(),
        confidence: 0.9,
        applicable: true,
    };
    ...
    let medium = EnvironmentMedium {
        ...
        current_state: EnvironmentState {
            ...
            pressure: EnvironmentPressure {
                level: 0.1,
                source: "none".into(),
                trend: environment::PressureTrend::Stable,
            },
...
fn test_update_mood() {
    ...
    engine
        .writer()
        .update_mood(EnvironmentMood::Stressed)
        .unwrap();
    assert_eq!(
        engine.reader().get_mood().unwrap(),
        EnvironmentMood::Stressed
    );
}
...
fn test_record_and_clear_incident() {
    ...
    let medium = EnvironmentMedium {
        ...
        current_state: EnvironmentState {
            ...
            pressure: EnvironmentPressure {
                level: 0.5,
                source: "traffic".into(),
                trend: environment::PressureTrend::Rising,
            },
            stability: StabilityAssessment {
                score: 0.8,
                window_secs: 300,
    ...
    engine
        .writer()
        .record_incident(ActiveIncident {
            id: "inc-1".into(),
            severity: "high".into(),
            summary: "CPU spike".into(),
            started_at: 2000,
            acknowledged: false,
        })
        .unwrap();
...
fn test_discover_capability() {
    ...
    let cap = Capability {
        ...
        category: CapabilityCategory::MachineLearning,
        available: true,
        constraints: vec![],
        confidence: 0.95,
    };
...
fn test_add_sensation() {
    ...
    engine
        ...
        .sense_resources(make_resource_body())
        .unwrap();
    let sensation = ResourceSensation {
...
fn test_clear_sensation() {
    ...
    engine
        .writer()
        .add_sensation(ResourceSensation {
...
fn test_detect_hallucination() {
    ...
    engine
        .writer()
        .detect_hallucination(DetectedHallucination {
            id: "h-1".into(),
...
fn test_add_downstream() {
    ...
    let dep_id = engine
        .writer()
        .add_downstream(DownstreamEntity {
            id: DependencyId::new(),
            service: ServiceId::new("postgres"),
            entity_type: DownstreamType::Database,
            health: HealthStatus::Healthy,
            latency: agentic_reality::types::resource::LatencyStats {
                p50_ms: 1.0,
                p95_ms: 5.0,
                p99_ms: 10.0,
                max_ms: 50.0,
...
fn test_link_causality() {
    ...
    let cause_id = engine
        .writer()
        .add_causal_event(CausalEvent {
            id: EventId::new(),
            event_type: CausalEventType::Action,
            description: "cause".into(),
            timestamp: 1000,
            causes: vec![],
            effects: vec![],
            metadata: HashMap::new(),
    ...
    engine
        .writer()
        .link_causality(&cause_id, &effect_id)
        .unwrap();
    let reader = engine.reader();
...
fn test_set_stakes_level() {
    ...
    engine
        .writer()
        .set_stakes_level(StakesLevel::High {
...
fn test_add_consequence() {
    ...
    engine
        .writer()
        .set_stakes_level(StakesLevel::Medium {
    ...
    engine
        ...
        .add_consequence(Consequence {
            effect: "data loss".into(),
            probability: 0.1,
            severity: Severity::Major,
            reversibility: Reversibility::Irreversible,
            affected: vec!["users".into()],
            latency_secs: Some(0),
        })
        .unwrap();
    let reader = engine.reader();
...
fn test_is_irreversible() {
    ...
    engine
        .writer()
        .set_stakes_level(StakesLevel::Critical {
    ...
    engine
        .writer()
        .add_irreversible_action(IrreversibleAction {
            action: "delete-database".into(),
            consequences: vec!["data loss".into()],
            safeguards: vec!["backup".into()],
...
fn test_add_guardrail() {
    ...
    engine
        .writer()
        .set_stakes_level(StakesLevel::Minimal {
            can_experiment: true,
        })
        .unwrap();
    engine
        ...
        .add_guardrail(Guardrail {
            name: "max-writes".into(),
            description: "limit writes per second".into(),
            active: true,
            triggered_count: 0,
        })
        .unwrap();
...
fn test_run_coherence_check() {
    ...
    engine
        .writer()
        .run_coherence_check(CoherenceCheck {
            check_type: CoherenceCheckType::TimeConsistency,
            passed: true,
            details: "ok".into(),
            timestamp: 1000,
            duration_ms: 5,
...
fn test_record_violation() {
    ...
    engine
        .writer()
        .record_violation(CoherenceViolation {
            violation_type: CoherenceCheckType::StateConsistency,
            detected: 1000,
            severity: 0.5,
            evidence: vec!["stale state".into()],
            possible_causes: vec!["clock drift".into()],
            resolution: None,
...
fn test_transition_lifecycle() {
    ...
    let transition = PendingTransition {
        id: TransitionId::new(),
        from: ContextId::new(),
        to_context: "staging".into(),
        transition_type: TransitionType::EnvironmentChange,
        phase: TransitionPhase::Planning,
        carry_over: vec!["state".into()],
        started: 1000,
    };
    let tid = engine.writer().begin_transition(transition).unwrap();
    engine
        .writer()
        .advance_transition(&tid, TransitionPhase::Executing)
        .unwrap();
    {
        let reader = engine.reader();
...
fn make_resource_body() -> ResourceBody {
    ResourceBody {
        mind: MindCapacity {
            ...
            feeling: MindFeeling::Clear,
            pressure: MemoryPressure::Low,
            largest_free_bytes: 10_000_000_000,
            fragmentation: 0.05,
            swap: None,
        },
        ...
        reach: NetworkReach {
            ...
            feeling: ReachFeeling::Connected,
