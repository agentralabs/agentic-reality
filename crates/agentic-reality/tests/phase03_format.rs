...
fn make_soul() -> DeploymentSoul {
    DeploymentSoul {
        ...
        birth: BirthContext {
            spawned_at: 1000, spawned_by: SpawnerIdentity { name: "test".into(), kind: SpawnerKind::Human, version: None },
            purpose: DeploymentPurpose { summary: "test".into(), category: PurposeCategory::Testing, specifics: HashMap::new() },
            expected_lifetime: None, previous_life: None, circumstances: BirthCircumstances::Virgin,
        },
        substrate: PhysicalSubstrate {
            id: "test".into(), tier: SubstrateTier::Laptop { owner: "dev".into(), os: "linux".into() },
            ...
            capabilities: SubstrateCapabilities { cpu_cores: 4, memory_mb: 8192, disk_gb: 256, gpu_available: false, network_bandwidth_mbps: None },
        },
        role: DeploymentRole { name: "test".into(), responsibilities: vec![], authority_level: AuthorityLevel::Contributor },
...
fn test_save_and_load_with_environment() {
    ...
    engine.writer().sense_environment(EnvironmentMedium {
        environment_type: EnvironmentType::Testing { test_type: "unit".into(), isolation: true },
        current_state: EnvironmentState {
            health: EnvironmentHealth { score: 1.0, components: HashMap::new() },
            pressure: EnvironmentPressure { level: 0.0, source: "".into(), trend: PressureTrend::Stable },
        ...
        properties: EnvironmentProperties { name: "test".into(), version: None, config: HashMap::new(), labels: HashMap::new() },
        ...
        inhabitants: vec!["agent".into()], boundaries: vec![], weather_history: vec![],
    }).unwrap();
    ArealWriter::save(&engine, &path).unwrap();
    let loaded = ArealReader::load(&path).unwrap();
    let reader = loaded.reader();
    let env = reader.get_environment().unwrap();
    assert!(matches!(env.environment_type, EnvironmentType::Testing { .. }));
...
fn test_save_and_load_roundtrip_full() {
    ...
    engine.writer().set_stakes_level(agentic_reality::types::stakes::StakesLevel::High { caution_required: true, approval_needed: true }).unwrap();
    engine.writer().run_coherence_check(agentic_reality::types::coherence::CoherenceCheck { check_type: agentic_reality::types::coherence::CoherenceCheckType::TimeConsistency, passed: true, details: "ok".into(), timestamp: 1000, duration_ms: 5 }).unwrap();
    ArealWriter::save(&engine, &path).unwrap();
    let loaded = ArealReader::load(&path).unwrap();
    assert!(loaded.is_initialized());
