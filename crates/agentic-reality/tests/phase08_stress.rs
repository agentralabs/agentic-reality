    ...
    DeploymentSoul {
        ...
        substrate: PhysicalSubstrate {
            ...
            network_position: NetworkPosition {
                ...
                hostname: Some("stress-test".into()),
                port: Some(8080),
                vpc: None,
                subnet: None,
            },
            isolation: IsolationLevel::Container,
            tenancy: TenancyModel::MultiTenant {
                tenant_id: "t-1".into(),
            },
        ...
        role: DeploymentRole {
            name: "stress-agent".into(),
            responsibilities: vec!["test".into()],
            authority_level: AuthorityLevel::Operator,
        },
        nature: ExistentialNature {
            cardinality: Cardinality::ReplicaOf { total: 3, index: 0 },
            expendability: 0.3,
            persistence: PersistenceModel::Persistent,
            statefulness: StatefulnessModel::EventSourced,
            clonability: true,
            primacy: InstancePrimacy::Primary,
        },
        lineage: DeploymentLineage {
            generation: 3,
            ancestors: vec![IncarnationId::new(), IncarnationId::new()],
            wisdom: vec![],
    ...
    for i in 0..20 {
        engine
            ...
            .add_downstream(DownstreamEntity {
                id: DependencyId::new(),
                service: ServiceId::new(format!("svc-{}", i)),
                entity_type: if i % 2 == 0 {
                    DownstreamType::Database
                } else {
    ...
    for i in 0..10 {
        engine
            ...
            .record_violation(CoherenceViolation {
                ...
                detected: 1000 + i,
                severity: 0.3,
                evidence: vec![format!("evidence-{}", i)],
                possible_causes: vec!["drift".into()],
                resolution: None,
...
fn test_full_state_roundtrip() {
    ...
    engine
        ...
        .sense_environment(EnvironmentMedium {
            ...
            inhabitants: vec!["agent-1".into(), "agent-2".into()],
            boundaries: vec![],
            weather_history: vec![],
        })
        .unwrap();
...
fn test_abort_transition() {
    ...
    let tid = engine
        .writer()
        .begin_transition(PendingTransition {
            id: TransitionId::new(),
            from: ContextId::new(),
            to_context: "new".into(),
            transition_type: TransitionType::Migration,
...
fn test_update_substrate() {
    ...
    engine
        .writer()
        .update_substrate(PhysicalSubstrate {
            id: "new".into(),
            tier: SubstrateTier::GpuCluster {
                gpu_count: 8,
                gpu_type: "A100".into(),
            ...
            isolation: IsolationLevel::Physical,
            tenancy: TenancyModel::Dedicated,
            capabilities: SubstrateCapabilities {
                cpu_cores: 96,
                memory_mb: 786432,
                disk_gb: 10000,
...
fn test_add_past_life() {
    ...
    engine
        .writer()
        .add_past_life(PastIncarnation {
            incarnation_id: IncarnationId::new(),
            birth: 500,
            death: DeathRecord {
                died_at: 999,
                cause: DeathCause::GracefulShutdown,
                graceful: true,
                state_preserved: true,
            },
            lessons: vec!["always save state".into()],
            substrate_tier: "Laptop".into(),
        })
        .unwrap();
    let reader = engine.reader();
...
fn test_risk_field() {
    ...
    risk_map.insert(
        RiskCategory::DataRisk,
        RiskLevel {
            score: 0.3,
            trend: RiskTrend::Stable,
            mitigations: vec!["encryption".into()],
        },
    );
    risk_map.insert(
        RiskCategory::SecurityRisk,
        RiskLevel {
            score: 0.5,
    ...
    engine
        .writer()
        .update_risk_field(RiskFieldPerception {
            overall_risk: 0.4,
            risk_map,
            hotspots: vec![],
            safe_zones: vec!["internal".into()],
            gradients: vec![],
            forecast: vec![],
        })
        .unwrap();
...
fn test_blast_radius() {
    ...
    engine
        .writer()
        .update_blast_radius(BlastRadiusAwareness {
            my_blast_radius: BlastRadius {
                affected_services: vec!["api".into()],
                affected_users: EstimatedImpact {
                    count: 1000,
                    percentage: 0.1,
                    confidence: 0.8,
                },
                affected_data: vec![],
                geographic_scope: vec!["us".into()],
            },
            dependency_blast: vec![],
            cascade: CascadeAnalysis {
                max_depth: 3,
                affected_count: 5,
                critical_path: vec!["db".into()],
