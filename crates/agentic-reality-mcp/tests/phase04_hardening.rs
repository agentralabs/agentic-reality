...
async fn test_null_operation_handled() {
    let s = session();
    let result = ToolRegistry::call("reality_deployment", Some(json!({"operation": null})), &s).await;
    assert!(result.is_ok());
}
...
async fn test_very_long_operation_name() {
    ...
    let result = ToolRegistry::call("reality_deployment", Some(json!({"operation": long_op})), &s).await;
    assert!(result.is_ok());
}
...
async fn test_nested_json_arguments() {
    let s = session();
    let result = ToolRegistry::call("reality_deployment", Some(json!({"operation": "get", "extra": {"nested": true}})), &s).await;
    assert!(result.is_ok());
}
...
async fn test_array_arguments() {
    let s = session();
    let result = ToolRegistry::call("reality_deployment", Some(json!({"operation": "get", "items": [1,2,3]})), &s).await;
    assert!(result.is_ok());
}
...
async fn test_all_tools_with_empty_operation() {
    ...
    for tool in &tools {
        ...
        assert!(result.is_ok(), "Tool '{}' panicked with empty operation", tool.name);
    }
}
...
async fn test_all_tools_with_null_args() {
    ...
    for tool in &tools {
        ...
        assert!(result.is_ok(), "Tool '{}' panicked with null args", tool.name);
    }
}
...
async fn test_unicode_operation() {
    let s = session();
    let result = ToolRegistry::call("reality_deployment", Some(json!({"operation": "获取"})), &s).await;
    assert!(result.is_ok());
}
...
async fn test_special_chars_operation() {
    let s = session();
    let result = ToolRegistry::call("reality_deployment", Some(json!({"operation": "get; DROP TABLE users;--"})), &s).await;
    assert!(result.is_ok());
}
...
async fn test_concurrent_tool_calls() {
    ...
    for i in 0..10 {
        ...
        handles.push(tokio::spawn(async move {
            ToolRegistry::call("reality_context", Some(json!({"operation": "summary"})), &s_clone).await
        }));
    }
