...
The system introduces 26~inventions organized into these seven tiers, persisted in the \texttt{.areal} binary format with per-section BLAKE3 checksums and LZ4 compression support.
...
Together, these 7~domains encompass 26~inventions that provide AI agents with existential grounding---a structured, continuous awareness of their relationship to the world they inhabit and affect.
...
The remainder of this paper is organized as follows. Section~\ref{sec:background} surveys related work in self-aware computing, situated cognition, and agent observability. Section~\ref{sec:architecture} presents the architecture, including the seven reality domains, 26~inventions, \texttt{.areal} binary format, and MCP integration. Section~\ref{sec:evaluation} reports benchmarks, test coverage, and robustness analysis. Section~\ref{sec:discussion} discusses implications, limitations, and future work. Section~\ref{sec:conclusion} concludes.
...
AgenticReality is organized as a layered architecture with seven reality domains containing 26~inventions total. A central \texttt{RealityEngine} manages cross-domain coherence, persistence via the \texttt{.areal} format, and exposure through 15~MCP tools.
...
\begin{tikzpicture}[
  ...
  domainbox/.style={rectangle, rounded corners=4pt, draw=#1, fill=#1!12, minimum width=\columnwidth-0.8cm, minimum height=0.55cm, font=\scriptsize\bfseries, align=center, thick},
  inventionlabel/.style={font=\tiny, text=argray, align=left},
]

% Stacked domain layers (bottom to top)
\node[domainbox=ardarkblue] (d7) at (0, 0) {7. Coherence Maintenance};
\node[inventionlabel, right] at (3.0, 0) {2 inv.};

\node[domainbox=arred] (d6) at (0, 0.85) {6. Stakes Perception};
\node[inventionlabel, right] at (3.0, 0.85) {3 inv.};

\node[domainbox=argreen] (d5) at (0, 1.70) {5. Temporal Grounding};
\node[inventionlabel, right] at (3.0, 1.70) {3 inv.};

\node[domainbox=arpurple] (d4) at (0, 2.55) {4. Topology Awareness};
\node[inventionlabel, right] at (3.0, 2.55) {4 inv.};

\node[domainbox=arblue] (d3) at (0, 3.40) {3. Reality Physics};
\node[inventionlabel, right] at (3.0, 3.40) {4 inv.};

\node[domainbox=arteal] (d2) at (0, 4.25) {2. Resource Proprioception};
\node[inventionlabel, right] at (3.0, 4.25) {5 inv.};

\node[domainbox=arorange] (d1) at (0, 5.10) {1. Deployment Consciousness};
\node[inventionlabel, right] at (3.0, 5.10) {5 inv.};

% Engine box at top
\node[rectangle, rounded corners=5pt, draw=argray, fill=argray!8, minimum width=\columnwidth-0.4cm, minimum height=0.65cm, font=\scriptsize\bfseries, thick] (engine) at (0, 6.15) {RealityEngine};

% Arrows from engine down
\draw[-{Stealth}, argray, thick] (engine.south) -- (d1.north);

% Persistence at bottom
\node[rectangle, rounded corners=3pt, draw=argray, dashed, minimum width=\columnwidth-0.4cm, minimum height=0.5cm, font=\scriptsize, thick] (storage) at (0, -0.85) {\texttt{.areal} binary format (BLAKE3 + LZ4)};
\draw[-{Stealth}, argray, dashed] (d7.south) -- (storage.north);

% MCP on the side
\node[rectangle, rounded corners=3pt, draw=arblue, fill=arblue!8, minimum width=1.4cm, minimum height=0.5cm, font=\tiny\bfseries, thick, rotate=90] (mcp) at (-3.8, 3.0) {15 MCP Tools};
\draw[-{Stealth}, arblue, thick] (mcp.south) -- ([xshift=-3.5cm]engine.west);
...
Table~\ref{tab:inventions} lists all 26~inventions organized by domain tier. Each domain addresses a distinct dimension of existential grounding.

\begin{table}[t]
\caption{The 26 inventions of AgenticReality, organized by domain tier. Each domain addresses a distinct dimension of existential grounding.}
\label{tab:inventions}
\centering
\scriptsize
\begin{tabular}{@{}clp{3.8cm}@{}}
\toprule
\textbf{Tier} & \textbf{Domain} & \textbf{Inventions} \\
\midrule
...
\begin{tikzpicture}[
  every node/.style={font=\scriptsize},
  block/.style={rectangle, draw=argray, minimum width=\columnwidth-1.0cm, minimum height=0.6cm, thick, align=center},
]
...
\end{tikzpicture}
\caption{The \texttt{.areal} binary format layout. A 256-byte header contains the ``REAL'' magic, version, flags, and incarnation ID. The section table indexes 9~section types, each with per-section BLAKE3 checksums. A 64-byte footer with ``REALEND\textbackslash 0'' magic provides format validation. LZ4 compression is optionally applied to section payloads.}
\label{fig:format}
\end{figure}
...
\begin{axis}[
  ...
  ymax=15000,
  grid=major,
  grid style={argray!20},
  nodes near coords,
  every node near coord/.append style={font=\tiny, anchor=south, rotate=0},
  point meta=explicit symbolic,
]
\addplot[fill=arblue!70, draw=arblue] coordinates {
  (create\_engine, 0.08) [0.08]
  (init\_soul, 0.03) [0.03]
  (get\_soul, 0.005) [{<}0.01]
  (ctx\_summary, 0.14) [0.14]
  (add\_anchor, 0.97) [0.97]
  (get\_anchors, 0.005) [{<}0.01]
  (set\_stakes, 0.01) [0.01]
  (is\_coherent, 0.005) [{<}0.01]
  (save\_10K, 4510) [4.5ms]
  (load\_10K, 3350) [3.4ms]
};
\end{axis}
\end{tikzpicture}
...
\midrule
Phase 1 & Core types, engine creation, soul lifecycle & 52 \\
Phase 2 & Resource proprioception, environment sensing & 48 \\
Phase 3 & Reality physics, anchors, freshness & 45 \\
Phase 4 & Topology, temporal grounding, causality & 42 \\
Phase 5 & Stakes perception, risk field, blast radius & 38 \\
Phase 6 & Coherence engine, transition manager & 35 \\
Phase 7 & MCP tools, JSON-RPC transport & 30 \\
Phase 8 & .areal format, persistence, integrity & 24 \\
\midrule
